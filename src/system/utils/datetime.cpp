...
DateTime Time_s::toDateTime() const {
  return DateTime(year, month, day, hour, minute, second, 0U);
}

Time_s Time_s::toUtcTime() const {
  DateTime time = this->toDateTime();
  time.add(DIFF_LOCAL_UTC_HOURS, DateTime::Period::Hour);
  return Time_s( time );
}
